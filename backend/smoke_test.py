...
def run_smoke_test() -> None:
    ...
    device_response = client.post(
        "/api/devices",
        headers={"Authorization": f"Bearer {login_token}"},
        json={"name": "My Phone", "platform": "android", "device_token": "device-abc-123"},
    )
    assert device_response.status_code == 200, device_response.text
    device_id = device_response.json()["id"]
    assert device_id > 0

    device_list_response = client.get(
        "/api/devices",
        headers={"Authorization": f"Bearer {login_token}"},
    )
    ...
    mark_lost_response = client.post(
        f"/api/devices/{device_id}/mark-lost",
        headers={"Authorization": f"Bearer {login_token}"},
    )
